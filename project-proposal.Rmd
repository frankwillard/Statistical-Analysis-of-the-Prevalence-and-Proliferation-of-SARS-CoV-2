...
``` {r COVID-19 Stringency, warning = FALSE, message = FALSE, echo = FALSE}
...
for (i in 2:nrow(covid1)) {
  ...
  if (j != 0 & i < nrow(covid1)) {
    covid1$new_cases_tomorrow[i] = covid1$new_cases[i + 1]
  }
  ...
  if (j > 1) {
    ...
    if (j > 14) {
      died_since <- covid1$total_deaths[i] - covid1$total_deaths[i - 14]
      recovered <- # You didn't die and you don't have covid anymore
      covid1$infected[i] <- covid1$infected[i - 1] + covid1$new_cases[i] - recovered
      covid1$recovered[i] <- covid1$population[i] - covid1$susceptible[i] - covid1$infected[i]
    }
  }
}
...
covid_sir <- covid1 %>%
  select(location, date, new_cases_tomorrow, stringency_index, population, susceptible, infected, recovered)

covid_sir

lm_sir <- lm(new_cases_tomorrow ~ susceptible + infected * stringency_index, data = covid_sir)
lm_sir %>%
 tidy() %>%
  arrange(p.value)
glance(lm_sir) %>% 
...
```{r Cor Test for the USA, warning = FALSE, message = FALSE, echo = FALSE}
...
```{r ML Test Summarize Data, warning = FALSE, message = FALSE, echo = FALSE}
...
```{r ML Split Data, warning = FALSE, message = FALSE, echo = FALSE}
...
```{r ML Find Lambda, warning = FALSE, message = FALSE, echo = FALSE}
...
```{r ML Run Lasso, warning = FALSE, message = FALSE, echo = FALSE}
...
```{r Eval Lasso, warning = FALSE, message = FALSE, echo = FALSE}
