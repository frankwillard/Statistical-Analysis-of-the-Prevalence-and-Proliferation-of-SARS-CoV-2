...
covid1$mu_stringency_index <- 0
covid1$mu_growth_new_cases <- 0
covid1$susceptible <- covid1$population - covid1$total_cases
covid1$infected <- covid1$total_cases - covid1$total_deaths
covid1$recovered <- covid1$total_deaths


for (i in 2:nrow(covid1)) {
  temp <- covid1$new_cases[i] / covid1$new_cases[i - 1]
  covid1$growth_new_cases[i] <- temp
  
  if (i > 14) {
    diff <- covid1$infected[i] - covid1$infected[i - 14]
    covid1$infected[i] <- diff
    covid1$recovered[i] <- covid1$recovered[i] + covid1$infected[i - 14]
...
``` {r visualizing-certain-countries}
...
covid_sir <- covid1 %>%
  select(location, new_cases, stringency_index, population, susceptible, infected, recovered)

covid_sir

lm_sir <- lm(new_cases ~ population + susceptible + infected * stringency_index, data = covid_sir)
lm_sir %>%
 tidy() %>%
  arrange(p.value)
glance(lm_sir) %>% 
...
covid_final_date_deaths <- covid_final_date_deaths %>% 
  select(deaths_per_case, -contains("case") & -contains("death") & -contains("test")) %>% 
  select(-(2:6)) %>% 
...
cols_reg <- colnames(covid_final_date_deaths)

cols <- cols_reg[cols_reg != 'deaths_per_case']
