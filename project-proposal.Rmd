...
covid1 <- covid1 %>%
  mutate(new_cases_cap = new_cases/population)
...
lm_all <- lm(growth_new_cases ~ total_cases + new_cases + total_deaths + new_deaths + stringency_index + population + population_density + median_age + aged_65_older + aged_70_older + gdp_per_capita + cardiovasc_death_rate + diabetes_prevalence + female_smokers + male_smokers + handwashing_facilities + hospital_beds_per_thousand + life_expectancy + latitude + human_development_index + extreme_poverty, data = covid_eur)
...
lm_hypothesis <- lm(growth_new_cases ~ stringency_index + population_density + latitude + human_development_index + extreme_poverty + total_cases, data = covid_eur)

lm_hypothesis %>%
 tidy()

glance(lm_hypothesis) %>% 
  select(adj.r.squared)

lm_heavy <- lm(growth_new_cases ~ female_smokers, data = covid_eur)
lm_heavy %>%
 tidy() %>%
  arrange(p.value)
