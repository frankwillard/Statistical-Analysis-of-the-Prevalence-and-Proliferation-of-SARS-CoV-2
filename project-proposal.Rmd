...
Intuitively, this makes sense. It is proven that social distancing, in all forms, is effective at slowing viral transmission. We are therefore interested to see the effects of policies mandating social distancing on the spread of Covid-19. To analyze the relationship, we will assess the correlation between a country's "stringency index" and the daily growth rate of its Covid-19 cases.

Stringency Index is a column in our dataset. It is a number on a scale of 0 to 100 that measures how strict a country's Covid-related policies are (100 being the strictest). It is a composite score of 9 other measurements including travel bans and workplace closures. 
...
We hypothesize that there is a negative correlation between stringency index and the daily growth rate of Covid cases. As stringency index increases, the daily growth rate of covid cases will decrease. We will test this hypothesis at the $\alpha = 0.05$ level.

$H_0$: The true correlation of a country's stringency index and total cases per capita is zero. ($\rho$ = 0)

$H_A$: The true correlation of a country's stringency index and total cases per capita is less than zero. ($\rho <$ 0)
...
``` {r Plotting Hypothesis 1}

covid_stringency <- covid_stringency %>%
  select(location, date, new_cases, growth_new_cases, stringency_index)

usa_only <- covid_stringency %>%
  filter(location == "United States") %>%
  mutate(preapril = if_else(date < "2020-04-01", T, F))

covid_stringency

ggplot(data = usa_only, aes(x=date, y = stringency_index, color = preapril)) +
  geom_line() +
  labs(x = "Date",
       y = "Stringency Index",
       title = "Stringency Index over time showing a sharp increase at the
       beginning of the pandemic in March with a slow decline afterward.") +
  
ggplot(data = usa_only, aes(x=date, y= growth_new_cases, color = preapril)) +
  geom_line() +
  labs(x = "Date",
       y = "Rate of Spread of Covid-19",
       title = "The transmission rate of Covid-19, showing a huge spike in
       transmission around mid-March, followed by a rapid decline
       to a transmission rate hovering around 1.0")  

```

```{r Cor Test for the USA}
...
# Pre April
cor.test(pre$growth_new_cases, pre$stringency_index, alternative="less")

# Post April
cor.test(post$stringency_index, post$growth_new_cases, alternative="less")
...
ggplot() +
  ...
  labs(title = "Chi-Square at Various Degrees of Freedom",
       x = "Chi-square",
       y = "Density") + geom_vline(xintercept = 28610)
...
ggplot() +
  ...
  labs(title = "Chi-Square at Various Degrees of Freedom",
       x = "Chi-square",
       y = "Density") + geom_vline(xintercept = 903.336)
