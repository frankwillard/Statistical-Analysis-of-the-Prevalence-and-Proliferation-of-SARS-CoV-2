...
for (i in 1:nrow(covid1)) {
  ...
  if (j > 1) {
    ...
    if (j > 14) {
      sum <- 0
      for (k in 0:14) {
        sum <- sum + covid1$new_cases[i - k] - covid1$new_deaths[i - k]
      }
      covid1$infected[i] <- sum
...
covid_sir <- covid1 %>%
  select(location, new_cases_tomorrow, stringency_index, population, susceptible, infected, recovered) %>%
  filter(recovered > 0)
...
``` {r checking-conditions}

# Augmented Linear Model
m_aug <- augment(lm_sir)

# Independence
ggplot(data = m_aug, 
       aes(x = 1:nrow(covid_sir), 
           y = .resid)) + 
  geom_point() + 
