...
covid <- covid %>%
  mutate(total_cases_per_cap = (total_cases/population),
...
```
Using Central Limit Theorem, we have a t distribution with test statistic t = 19.339 and 40315 degrees of freedom. Because our calculated p-value of 1 is greater than my predetermined $\alpha$ of 0.05, we fail to reject the null hypothesis. There is not enough evidence to suggest any negative correlation between stringency index and total cases per capita.
...
Variables with strongest correlation: male_smokers, diabetes_prevalence, hospital_beds_per_thousand, aged_70_older, population_density

gdp_per_capita, human_development_index, cardiovasc_death_rate had lower correlations than we expected
