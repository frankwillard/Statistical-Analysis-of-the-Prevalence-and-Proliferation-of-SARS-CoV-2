...
deaths = c(rep(NA, nrow(covid)))

covid_final_month <- covid %>%
  filter(date > "2020-09-05")

for (i in 1:nrow(covid_final_month)) {
  day <- covid_final_month$date[i]
  covid_temp <- filter
  deaths[i] = covid_final_date$deaths_per_case[i]
  perc90 <- quantile(covid$deaths_per_case[i], 0.9)
  perc70 <- quantile(covid$deaths_per_case[i], 0.7)
  perc30 <- quantile(covid$deaths_per_case[i], 0.3)
  perc10 <- quantile(covid$deaths_per_case[i], 0.1)
  covid$dpc_cat[i] <- case_when(
    covid$deaths_per_case[i] >= perc90 ~ "Very High",
    covid$deaths_per_case[i] >= perc70 ~ "High",
    covid$deaths_per_case[i] >= perc30 ~ "Average",
    covid$deaths_per_case[i] >= perc10 ~ "Low",
    covid$deaths_per_case[i] >= 0 ~ "Very Low"
  )
}



covid_hdic <- covid_final_date %>%
...
chisq.test(table(covid_hdic$hdi_columns, covid_hdic$dpc_cat))
...
chisq.test(table(covid_aged$aged_columns, covid_aged$dpc_cat))
