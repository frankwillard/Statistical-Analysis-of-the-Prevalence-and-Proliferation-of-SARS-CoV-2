...
ggplot(covid_map, mapping = aes(long, lat, group = group)) +
  ...
  labs(title = "Median stringency index by country suggesting that most 
       countries have a median stringency index of about 60", 
...
ggplot(covid_map, mapping = aes(long, lat, group = group)) +
  ...
  labs(title = "Total cases per capita by country suggesting that total cases
  per capita varies largely by continent", x = "Longitude", y = "Latitude")
...
```{r Visualize HDI vs Deaths/Case Over Time}
...
chisq_hdi <- chisq.test(table(covid_hdic$hdi_columns, covid_hdic$deaths_per_case))
...
chisq_aged <- chisq.test(table(covid_aged$aged_columns, covid_aged$deaths_per_case))
...
```{r Deaths Per Case Multiple Linear Regression}
